...
const dataFile = "./data-deleted.csv"; // Path to the data CSV file
const outputDemographicsFile = "./validDemographics.csv"; // Path for the output demographics CSV file
const outputDataFile = "./data-sex.csv"; // Path for the updated data CSV file
...
filteredDemographicsData.forEach((row) => {
  participantSexMap[row["Participant id"]] = row.Sex;
});

// Add 'Sex' column to data-deleted data
const updatedDataDeletedData = dataDeletedData.map((row) => ({
  ...row,
  Sex: participantSexMap[row.prolificId] || "", // Fallback to empty string if no match found
}));
